import { Ft as assertInInjectionContext, G as Injector, O as DestroyRef, Ur as isSignal, W as InjectionToken, aa as ɵɵdefineInjector, c as computed, ca as ɵɵinject, ia as ɵɵdefineInjectable, ni as makeEnvironmentProviders, xi as runInInjectionContext, yr as inject } from "./_resource-chunk-Bs197SVl.js";
...
var DefaultLoader = class {
	...
	getTranslation(lang) {
		return (0, import_cjs.of)(this.translations.get(lang) || {});
	}
};
...
function getFallbacksLoaders({ mainLoader, path, data, fallbackPath, inlineLoader }) {
	return (fallbackPath ? [path, fallbackPath] : [path]).map((path) => {
		return (0, import_cjs.from)(resolveLoader({
			path,
			mainLoader,
			inlineLoader,
			data
		})).pipe((0, import_cjs.map)((translation) => ({
			translation,
			lang: path
		})));
	});
...
var TranslocoService = class TranslocoService {
	...
	*/
	...
	constructor(loader, parser, missingHandler, interceptor, userConfig, fallbackStrategy) {
		...
		this.lang = new import_cjs.BehaviorSubject(this.getDefaultLang());
	...
	*/
	...
	load(path, options = {}) {
		if (this.destroyed) return import_cjs.EMPTY;
		...
		if (this.useFallbackTranslation(path)) {
			...
			loadTranslation = (0, import_cjs.forkJoin)(getFallbacksLoaders({
				...loadersOptions,
				fallbackPath: fallback
			}));
		} else loadTranslation = (0, import_cjs.from)(resolveLoader(loadersOptions));
		const load$ = loadTranslation.pipe((0, import_cjs.retry)(this.config.failedRetries), (0, import_cjs.tap)((translation) => {
			if (Array.isArray(translation)) {
				translation.forEach((t) => {
					this.handleSuccess(t.lang, t.translation);
					if (t.lang !== path) this.cache.set(t.lang, (0, import_cjs.of)({}));
				});
				return;
			}
			this.handleSuccess(path, translation);
		}), (0, import_cjs.catchError)((error) => {
			if (typeof ngDevMode !== "undefined" && ngDevMode) console.error(`Error while trying to load "${path}"`, error);
			return this.handleFailure(path, options);
		}), (0, import_cjs.shareReplay)(1), takeUntilDestroyed(this.destroyRef));
	...
	*/
	selectTranslate(key, params, lang, _isObject = false) {
		let inlineLoader;
		const load = (lang, options) => this.load(lang, options).pipe((0, import_cjs.map)(() => _isObject ? this.translateObject(key, params, lang) : this.translate(key, params, lang)));
		if (o(lang)) return this.langChanges$.pipe((0, import_cjs.switchMap)((lang) => load(lang)));
		...
		const scope = lang;
		return this.langChanges$.pipe((0, import_cjs.switchMap)((lang) => load(`${scope}/${lang}`, { inlineLoader })));
	}
	/**
	...
	*/
	...
	selectTranslateObject(key, params, lang) {
		...
		return this.selectTranslateObject(firstKey, firstParams, lang).pipe((0, import_cjs.map)((value) => {
	...
	*/
	selectTranslation(lang) {
		...
		if (lang) {
			...
			if (this.isLang(lang) || scopeLangSpecified) language$ = (0, import_cjs.of)(lang);
			else language$ = this.langChanges$.pipe((0, import_cjs.map)((currentLang) => `${lang}/${currentLang}`));
		}
		return language$.pipe((0, import_cjs.switchMap)((language) => this.load(language).pipe((0, import_cjs.map)(() => this.getTranslation(language)))));
	}
	/**
	...
	*/
	_loadDependencies(path, inlineLoader) {
		...
		if (this._isLangScoped(path) && !this.isLoadedTranslation(mainLang)) return (0, import_cjs.combineLatest)([this.load(mainLang), this.load(path, { inlineLoader })]);
...
function listenOrNotOperator(listenToLangChange) {
	return listenToLangChange ? (source) => source : (0, import_cjs.take)(1);
}
...
var TranslocoDirective = class TranslocoDirective {
	...
	ngOnInit() {
		...
		this.service.langChanges$.pipe((0, import_cjs.switchMap)((activeLang) => {
			const lang = this.langResolver.resolve({
				inline: this.inlineLang,
				provider: this.providerLang,
				active: activeLang
			});
			return Array.isArray(this.providerScope) ? (0, import_cjs.forkJoin)(this.providerScope.map((providerScope) => this.resolveScope(lang, providerScope))) : this.resolveScope(lang, this.providerScope);
...
var TranslocoPipe = class TranslocoPipe {
	...
	transform(key, params, inlineLang) {
		...
		this.subscription = this.service.langChanges$.pipe((0, import_cjs.switchMap)((activeLang) => {
			const lang = this.langResolver.resolve({
				inline: inlineLang,
				provider: this.providerLang,
				active: activeLang
...
var TestingLoader = class TestingLoader {
	...
	getTranslation(lang) {
		return (0, import_cjs.of)(this.langs[lang]);
	}
...
function translateSignal(key, params, lang, injector) {
	...
	return toSignal(runInInjectionContext(injector, () => {
		...
		return computerKeysAndParams(key, params).pipe((0, import_cjs.switchMap)((dynamic) => service.selectTranslate(dynamic.key, dynamic.params, scope)));
	}), {
...
function translateObjectSignal(key, params, lang, injector) {
	...
	return toSignal(runInInjectionContext(injector, () => {
		const service = inject(TranslocoService);
		const scope = resolveScope(lang);
		return computerKeysAndParams(key, params).pipe((0, import_cjs.switchMap)((dynamic) => service.selectTranslateObject(dynamic.key, dynamic.params, scope)));
	}), {
		initialValue: Array.isArray(key) ? [] : {},
		injector
	});
...
function computerKeysAndParams(key, params) {
	if (!isSignalKey(key) && !isSignalParams(params)) return (0, import_cjs.of)({
		key,
		params
	});
