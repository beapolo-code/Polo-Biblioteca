import { l as __spreadArray, o as __extends, t as __assign } from "./tslib.es6-BIDSHNF3.js";
import { D as parse, a as addDays, p as createDuration } from "./headless-calendar-CCBEW-nM.js";
...
//#endregion
//#region node_modules/@fullcalendar/core/protected-api.js
...
var recurringType = {
	...
	expand(eventRRuleData, framingRange, calendarDateEnv) {
		return eventRRuleData.rruleSet.between(addDays(framingRange.start, -1), addDays(framingRange.end, 1)).map((date) => {
			return calendarDateEnv.createMarker(eventRRuleData.dateEnv ? eventRRuleData.dateEnv.toDate(date) : date);
		});
	}
};
function parseEventRRule(eventProps, dateEnv) {
	...
	for (let exdateInput of exdateInputs) {
		...
		rruleSet.exdate(/* @__PURE__ */ new Date(res.marker.valueOf() - (res.timeZoneOffset || 0) * 60 * 1e3));
	}
...
function parseRRuleObject(rruleInput, dateEnv) {
	...
	function processDateInput(dateInput) {
		if (typeof dateInput === "string") {
			...
			if (markerData) {
				...
				return /* @__PURE__ */ new Date(markerData.marker.valueOf() - (markerData.timeZoneOffset || 0) * 60 * 1e3);
			}
			return null;
		}
		return dateInput;
	...
	return {
		rrule: new RRule({
			...
			wkst: rruleInput.wkst == null ? (dateEnv.weekDow - 1 + 7) % 7 : convertConstant(rruleInput.wkst),
...
function parseRRuleString(str) {
	return {
		rruleSet: rrulestr(str, { forceset: true }),
		...analyzeRRuleString(str)
	};
}
...
function convertConstant(input) {
	if (typeof input === "string") return RRule[input.toUpperCase()];
	return input;
}
var index = {
	name: "rrule",
